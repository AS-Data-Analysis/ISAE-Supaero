...
def mass_breakdown_sun_plot_drone(drone_file_path: str, file_formatter=None):
    ...
    # MTOW
    MTOW = variables["optim:objective:MTOW"].value[0]
...
def drone_geometry_plot(
    ...
    MTOW = variables["optim:objective:MTOW"].value[0]  # [kg]
    A_body = 0.045 * (MTOW / 1.4) ** (2 / 3)  # variables["data:structure:body:surface:top"].value[0]  # [m**2]
    ...
    Vol_bat = 0.00106 * variables["data:battery:voltage"].value[0] * variables["data:battery:capacity"].value[0] * 0.000001  # [m**3]
    m_mot = variables["data:motor:mass"].value[0]  # [kg]
    Lmot = 0.088 * (m_mot / 0.575)**(1/3)  # [m]
