...
ra = 20*ACCVAR(2:3); %Bruit de mesure accéléro.
% The accelerometer gives inaccurate readings when moved horizontally fast, assuming it as a rotation
% Hence, we have less confidence in Accelerometer than the Gyrometer
...
while(1)
    ...
    F = 1; % Unlike for 1 state X = theta, when the value of F will be just 1, on including theta_derivative,
    ...
    if ~isnan(t)
        ...
        Yhat = [-sin(X(1)) % From dynamic equations
            cos(X(1))];
        H = [-cos(X(1)) % On differentiating to linearize
            -sin(X(1))]; % To convert to degrees
        S = H*P*H'+R;
        K = P*H'*inv(S);
        X = X+K*(Y-Yhat);
        P = P-K*S*K';
